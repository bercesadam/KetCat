...
def setup_bloch(axis):
    ...
    y = np.sin(u)*np.sin(v)
    z = np.cos(v)
    axis.plot_wireframe(x, y, z, color="gray", alpha=0.5, linewidth=0.5)
...
def update(frame):
    ...
    # STIRAP update
    start = 0
    print(times[frame], stirap_data[frame, 1], stirap_data[frame, 3])
    ...
    text_l1.set_text(f"Pump: {stirap_data[frame, 0]:.1f} nm")
    text_l2.set_text(f"Stokes: {stirap_data[frame, 2]:.1f} nm")
